...
from smart_wheelchair_safety.neupan_wheelchair_node import (
    NeuPANWheelchairNode, scan_points, transform_points, reference_points,
)


class NodeTest(unittest.TestCase):
    def test_scan_fov_range_and_finite_filter(self):
        scan = NS(ranges=[1.] * 37, angle_min=-math.pi,
                  angle_increment=math.pi / 18, range_min=.1, range_max=10.)
        for side, bounds in [('left', (-30, 170)), ('right', (-170, 30))]:
            points = scan_points(scan, side, 5.)
        ...
        scan.ranges = [float('nan'), float('inf'), .01, 6.]
        self.assertEqual(len(scan_points(scan, 'right', 5.)), 0)

    def test_world_transform_and_reference(self):
        transform = NS(translation=NS(x=10., y=20.),
                       rotation=NS(x=0., y=0., z=math.sin(math.pi/4), w=math.cos(math.pi/4)))
        np.testing.assert_allclose(transform_points([[1., 0.]], transform), [[10., 21.]])
        pose = NS(position=NS(x=10., y=21.), orientation=transform.rotation)
        np.testing.assert_allclose(reference_points(NS(poses=[NS(pose=pose)])), [[10., 21., math.pi/2]])
    ...
    def node(self):
        ...
        node.input_timeout = .25
        node._reference = (10., 10., np.array([[10., 20., 0.], [11., 20., 0.]]))
        node._odom = (10., 10., np.array([10., 20., 0.]))
        node._scans = {side: (10., 10., np.array([[12., 20.]])) for side in ('left', 'right')}
        node._ros_now = lambda: self.now
        node.adapter = NS(reason='ok', set_obstacles=lambda *a, **kw: None,
                          set_initial_path=lambda *a: None,
                          step=lambda *a, **kw: (np.array([.4, .1]), np.array([[10., 20., 0.]])))
        self.results = []
    ...
    def test_fresh_result_keeps_reference_stamp(self):
        node = self.node()
        node.tick()
        np.testing.assert_allclose(self.results[-1][0], [.4, .1])
    ...
    def test_missing_or_stale_each_input_invalidates_action(self):
        for field in ('reference', 'odom', 'left', 'right'):
            ...
            if field in ('left', 'right'):
                node._scans[field] = (9., 9., np.empty((0, 2)))
            else:
    ...
    def test_expired_during_solve_invalidates_action(self):
        ...
        def slow(*args, **kwargs):
            self.now += .3
            return np.array([.4, .1]), np.array([[10., 20., 0.]])
    ...
    def test_receipt_of_old_or_future_stamp_does_not_refresh_input(self):
        node = self.node()
        for stamp in (9., 11., 0.):
            node._odom = (10., stamp, np.zeros(3))
            node.tick()
            self.assertIsNone(self.results[-1][2])
