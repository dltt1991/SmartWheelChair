...
- 后轮差速驱动，两侧后轮独立动力输出。
- 两个被动前万向轮。
- 左右两颗 360 度旋转单线激光雷达，仿真输出完整 2D 扫描供安全检查，规划器使用侧边/侧前视场，控制器剔除车身回波。
- 后方中部 120 度广角摄像头。
...
两颗激光雷达均发布完整 360 度扫描：每侧 721 束，角分辨率 0.5 度，10 Hz。安全检查使用完整扫描，避免原先裁剪为 200 度产生的后方盲区；沿墙/过门规划仍使用原侧边和侧前约 200 度视场。Gazebo 原生扫描显示为淡透明色；ROS 输出是 `LaserScan` 话题，不是 3D 点云：
...
原生蓝色雷达扇面与射线使用 `3.5%` 不透明度的淡色材质，只改变 GUI 显示；雷达量程、扫描频率和后摄画面不变。
...
- 前轮是被动万向轮。
- 后轮独立驱动，组成差速底盘。
- 两颗 360 度旋转单线激光雷达提供经过车身过滤的完整安全检查输入，规划分支使用侧边/侧前局部视场。
